...
def build_graph(df, dataset):
    ...
    #####
    ## CUT 6: Make cut on TAU-quark probabilities
    #####
    df = df.Define("recojet_isTAU_jet0", "recojet_isTAU[jet_indx[0]]")
    df = df.Define("recojet_isTAU_jet1", "recojet_isTAU[jet_indx[1]]")
    results.append(df.Histo1D(("recojet_isTAU_jet0", "", *bins_score), "recojet_isTAU_jet0"))
    results.append(df.Histo1D(("recojet_isTAU_jet1", "", *bins_score), "recojet_isTAU_jet1"))
    df = df.Filter("recojet_isTAU_jet0 > 0.95 && recojet_isTAU_jet1 > 0.95")
