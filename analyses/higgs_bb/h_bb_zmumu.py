...
jetClusteringHelper2 = helper_jetclustering.ExclusiveJetClusteringHelper(njets, collection="rps_no_muons")
...
def build_graph(df, dataset):
    ...
    probabilities=np.linspace(0.7, 1, 10)
    for i, probability in enumerate(probabilities):
        index=i+9
        df = df.Filter(f"recojet_isB_jet0 > {probability} && recojet_isB_jet1 > {probability}")
        results.append(df.Histo1D((f"b_prob_{probability}", "", *bins_prob), f"cut{index}"))
    ...
    df = df.Define("jets_mc", "FCCAnalyses::jetTruthFinder(_jetc, rps_no_muons, Particle, MCRecoAssociations1)")
